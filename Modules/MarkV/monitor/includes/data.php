...
if(file_exists("/sd/var/lib/vnstat/") && !file_exists("/var/lib/vnstat/"))
{
	exec("ln -s /sd/var/lib/vnstat /var/lib/vnstat");
}
...
if (isset($_GET['large'])) 
{
	...
	echo '<div id="monitor" class="tab"><ul>';
	for($i=0;$i<count($interfaces);$i++)
	{
...
else if (isset($_GET['small'])) 
{
	...
	echo '<div id="monitor2" class="tab"><ul>';
	for($i=0;$i<count($interfaces);$i++)
	{
		if($i == 0) $class = "selected"; else $class = "";
		$tmp_int = str_replace(".", "_", $interfaces[$i]);
