...
if (isset($_GET['p0f']))
{
	if($is_p0f_installed)
	{
		if (isset($_GET['start']))
		{
			...
			$time = time();
			$full_cmd = "p0f -i ".$interface_run." -o ".$directory."includes/log/output_".$time.".log";

			shell_exec("echo \"#!/bin/sh\n".$full_cmd."\" > ".$directory."includes/p0f.sh && chmod +x ".$directory."includes/p0f.sh &");
			exec("echo ".$directory."includes/p0f.sh | at now");
		}
		if (isset($_GET['stop']))
		{
			exec("kill `ps -ax | grep p0f | grep -v -e grep | grep -v -e php | awk {'print $1'}`");
...
if (isset($_GET['load']))
{
	if (isset($_GET['file']))
	{
		if (isset($_GET['what']) && $_GET['what'] == "log")
		{
			...
			echo "<strong>p0f log ".$_GET['file']." [".$log_date."]</strong><br/><br/>";
		
			echo '<textarea class="p0f" cols="85" rows="29">';
		...
		if (isset($_GET['what']) && $_GET['what'] == "custom")
		{
			...
			echo "<strong>p0f custom log ".$_GET['file']." [".$log_date."]</strong><br/><br/>";
...
if (isset($_GET['boot']))
{
	if (isset($_GET['action']))
	{
		...
		switch($action)
		{
			...
			case 'disable': 
				exec("sed -i '/p0f\/includes\/autostart.sh/d' /etc/rc.local");
			break;
		}
	}	
}
...
if (isset($_GET['execute']))
{
	if (isset($_GET['cmd']))
	{	
		$time = time(); $cmd = base64_decode($_GET['cmd']);
...
if (isset($_GET['install'])) 
{
	if (isset($_GET['where']))
	{
		...
		{
			case 'sd': 
				exec("opkg update && opkg install p0f --dest sd"); 
			break;
			
			case 'internal': 
				exec("opkg update && opkg install p0f"); 
			break;
		}
	}
}
