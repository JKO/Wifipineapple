...
if (isset($_GET['history']))
{
	...
	for($i=0;$i<count($log_list);$i++)
	{
		if(basename($log_list[$i]) != "tmp")
		{
			...
			echo "<a href=\"/components/infusions/sitesurvey/includes/actions.php?_csrfToken=".$_SESSION['_csrfToken']."&download&log&file=".basename($log_list[$i])."\">download</a> | ";
...
if (isset($_GET['captures']))
{
	...
	for($i=0;$i<count($captures_list);$i++)
	{
		...
		echo gmdate('Y-m-d H-i-s', $info[1])." [".$BSSID." - ".$ESSID."] #IVS ".$IVS." ";
		...
		for($j=0;$j<count($output_types);$j++)
		{
			...
			$custom_command = replace_tags($tags, $custom_commands[1]);
			
			echo $output_types[$j]." ";
			echo "[<a href=\"/components/infusions/sitesurvey/includes/actions.php?_csrfToken=".$_SESSION['_csrfToken']."&download&capture&file=".$file."\">download</a> | ";
			echo "<a href=\"javascript:sitesurvey_execute_custom_script('".base64_encode($custom_command)."');\">exec</a>] ";
		}
		echo "<br /><br />";
	}
}
