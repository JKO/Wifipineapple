...
if (isset($_GET['load']))
{
	if (isset($_GET['file']))
	{
		$log_date = gmdate("F d Y H:i:s", filemtime($directory."includes/scans/".$_GET['file']));
		echo "<strong>nmap log ".$_GET['file']." [".$log_date."]</strong><br/><br/>";
		
		echo '<textarea class="nmap" cols="85" rows="29">';
...
if (isset($_GET['install'])) 
{
	if (isset($_GET['where']))
	{
		...
		{
			case 'sd': 
				exec("opkg update && opkg install nmap --dest sd"); 
			break;
